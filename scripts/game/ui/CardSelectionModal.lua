local sceneName = "CardSelectionModal"
...
function scene:create( event )
   ...
   self.params.min = self.params.min or (self.params.isOrdering and #self.params.cards) or 0
   ...
   self.selectButton = Button:new({
      label = self.params.isSelection ~= false and not self.params.isOrdering and "Select" or "OK",
      font = buttonFont,
...
function scene:setupCards(cards)
   ...
   local function touchListener(event)
      if event.phase == "ended" and not event.target.held and event.target.isEnabled and self.params.isSelection ~= false then
...
function scene:update()
   ...
   if params.min > #selectedCards then
   ...
   for k,card in pairs(self.cards) do
      ...
      elseif card.isEnabled or self.params.isSelection == false then
